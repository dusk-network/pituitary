...
// IsSpecPathNotFound reports whether err is a missing indexed-spec path lookup.
func IsSpecPathNotFound(err error) bool {
	var target *specPathNotFoundError
	return errors.As(err, &target)
}

// ResolveIndexedSpecRefWithConfigContext resolves one indexed spec ref from a path selector.
...
// ResolveIndexedSpecRefsWithConfigContext resolves indexed spec refs from workspace-relative or absolute paths.
