...
func TestRebuildDefaultsInferAppliesToBasedOnCorpus(t *testing.T) {
	t.Parallel()

	cases := []struct {
		name            string
		tomlFragment    string
		specAppliesTo   string
		specBody        string
		withCodeFile    bool
		wantEnabled     bool
		wantInferredGT0 bool
	}{
		{
			name:         "unset_with_code_applies_to_defaults_on",
			tomlFragment: "",
			// Manual applies_to pins a different file so it does not
			// dedupe the symbol-matched inferred edge on src/limiter.go.
			specAppliesTo:   `applies_to = ["code://src/manual_target.go"]` + "\n",
			specBody:        "This spec governs the SlidingWindowLimiter implementation.\n",
			withCodeFile:    true,
			wantEnabled:     true,
			wantInferredGT0: true,
		},
		{
			name:            "unset_without_code_applies_to_stays_off",
			tomlFragment:    "",
			specAppliesTo:   "",
			specBody:        "plain spec body with no code references.\n",
			withCodeFile:    false,
			wantEnabled:     false,
			wantInferredGT0: false,
		},
		{
			name:            "explicit_false_wins_over_code_applies_to",
			tomlFragment:    "infer_applies_to = false\n",
			specAppliesTo:   `applies_to = ["code://src/limiter.go"]` + "\n",
			specBody:        "This spec governs the SlidingWindowLimiter implementation.\n",
			withCodeFile:    true,
			wantEnabled:     false,
			wantInferredGT0: false,
...
body = "body.md"
`+tc.specAppliesTo)
			mustWriteFile(t, filepath.Join(dir, "specs", "rate-limit", "body.md"), tc.specBody)
...
`)
			...
			if result.InferAppliesToEnabled != tc.wantEnabled {
				t.Errorf("InferAppliesToEnabled = %v, want %v", result.InferAppliesToEnabled, tc.wantEnabled)
			}
			if tc.wantInferredGT0 && result.InferredEdgeCount == 0 {
				t.Errorf("InferredEdgeCount = 0, want > 0 (default should run inference)")
			}
			if !tc.wantInferredGT0 && result.InferredEdgeCount != 0 {
				t.Errorf("InferredEdgeCount = %d, want 0", result.InferredEdgeCount)
			}
		})
	}
}
