...
func TestRunCommandHelpAcrossSurface(t *testing.T) {
	...
	testCases := map[string][]string{
		...
		"analyze-impact":    {"usage: pituitary [--config PATH] analyze-impact (--path PATH | --spec-ref REF | --request-file PATH|-) [--change-type TYPE] [--format FORMAT]", "shared config resolution:", "--path VALUE", "--change-type VALUE", "--request-file VALUE"},
		...
		"check-doc-drift":   {"usage: pituitary [--config PATH] check-doc-drift [--doc-ref REF]... [--scope all] [--request-file PATH|-] [--format FORMAT]", "shared config resolution:", "--doc-ref VALUE", "--scope VALUE", "--request-file VALUE"},
